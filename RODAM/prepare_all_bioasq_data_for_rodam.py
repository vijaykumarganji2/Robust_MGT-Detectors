...
def prepare_ovr_datasets(src_dataset_path = '../../data/perturbed_data', src_llm_name = 'GPT', tgt_dataset_path = '../../data/HealthCare/perturbed_bioasq', storing_data_path = '../../data/RODAM_Data/bioasq_setup/gpt'):
    for file_name in ['train.jsonl', 'valid.jsonl', 'test.jsonl']:
        source_df = pd.DataFrame()
           
        for dataset in datasets:
            df = pd.read_json(f"{src_dataset_path}/{src_llm_name}_Version/{dataset}/{file_name}",
        ...
        target_df = pd.read_json(f"{tgt_dataset_path}/bioasq_{file_name}", orient='records',lines=True)
