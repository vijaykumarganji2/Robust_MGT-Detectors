...
class ContrastivelyInstructedRoberta(torch.nn.Module):
    ...
    def forward(self, src_texts:torch.Tensor, src_masks:torch.Tensor, src_texts_perturb:torch.Tensor, src_masks_perturb:torch.Tensor, \
        ...
        loss = (1-self.lambda_w)*(src_LCE_real + src_LCE_perturb)/2 + lambda_mmd*mmd ##loss without constractive loss
        ...
        '''if not use_both_ce_losses:
            ...
            if use_ce_perturb:
                ...
                loss = (1-self.lambda_w)*src_LCE_real + self.lambda_w*(src_lctr+tgt_lctr)/2 + lambda_mmd*mmd # loss without ce_perturb
'''
...
src_data_dir = './data/RODAM_Data/OVR_Dataset/GPT_Version/squad_OVR/source_data'
tgt_data_dir = './data/RODAM_Data/OVR_Dataset/GPT_Version/squad_OVR/target_data'
src_real_dataset = 'real'
src_fake_dataset = 'fake'
tgt_real_dataset = 'real'
tgt_fake_dataset = 'fake'
model_save_path = './Trained_Models/Ablation_Study/RODAM/'
model_save_name = f'rodam_squad_gpt_ovr_without_constractive_loss.pt'
checkpoint_name = f'checkpoint_rodam_squad_gpt_ovr_without_constractive_loss.pt'
