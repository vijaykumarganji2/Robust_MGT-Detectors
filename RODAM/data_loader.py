...
class Corpus:
    def __init__(self, name, data_dir='data', label=False, skip_train=False, single_file=False):
        ...
        if single_file:
            ...
            self.valid, self.valid_perturb = load_texts(f'{data_dir}/{name}.valid.jsonl')


class EncodedFakeDataset(Dataset):
    def __init__(self,
    ...
    def __len__(self):
        return len(self.fake_texts)

    def __getitem__(self, index):

        text = self.fake_texts[index]
        text_perturb = self.fake_text_perturb[index]
        label = 0
