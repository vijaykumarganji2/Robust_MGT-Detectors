...
class ContrastivelyInstructedRoberta(torch.nn.Module):
    ...
    def forward(self, src_texts:torch.Tensor, src_masks:torch.Tensor, src_texts_perturb:torch.Tensor, src_masks_perturb:torch.Tensor, \
        ...
        loss = (1-self.lambda_w)*(src_LCE_real + src_LCE_perturb)/2 ## loss with only cross entropy loss
...
src_data_dir = './data/RODAM_Data/OVR_Dataset/GPT_Version/wp_OVR/source_data'
tgt_data_dir = './data/RODAM_Data/OVR_Dataset/GPT_Version/wp_OVR/target_data'
...
model_save_name = f'rodam_wp_gpt_ovr_with_only_ce.pt'
checkpoint_name = f'checkpoint_rodam_wp_gpt_ovr_with_only_ce.pt'
